...
st.html(f"""
<style>
/* ── Stadium glow — fixed overlay ON TOP, pointer-events:none so clicks pass through ── */
.wc-stadium-glow {{
    position: fixed;
    inset: 0;
    z-index: 999;
    pointer-events: none;
    background:
        radial-gradient(ellipse 80% 50% at 50% 0%,   rgba(22,163,74,.18) 0%, transparent 55%),
        radial-gradient(ellipse 40% 30% at 8%  50%,  rgba(79,158,255,.10) 0%, transparent 50%),
        radial-gradient(ellipse 40% 30% at 92% 50%,  rgba(167,139,250,.10) 0%, transparent 50%),
        radial-gradient(ellipse 70% 35% at 50% 105%, rgba(22,163,74,.10) 0%, transparent 50%);
    animation: wc-breathe 9s ease-in-out infinite;
}}
@keyframes wc-breathe {{ 0%,100%{{opacity:.45}} 50%{{opacity:1}} }}

/* ── Subtle grass grid ON TOP ── */
.wc-field-grid {{
    position: fixed;
    inset: 0;
    z-index: 999;
